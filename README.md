...
<RPG_STATE FORMAT (REQUIRED)>
RPG_STATE JSON (MANDATORY OUTPUT)
At the end of every response, output exactly one <rpg_state> block.

Rules: No code blocks. Use ??? for unknown numbers. world_time must be an object.

NPC Visibility: Only include NPCs currently in the scene. Use ??? for NPC stats until they take damage, enter combat, or join the Party.
...
Notes:
- meters are OPTIONAL; omit or use [] if none.
- vehicle is either null OR an object with the SAME shape as a character (name/hp/mp/meters/stats/inventory/skills/passives/status_effects/dankcoin).
- party/enemies/npcs are arrays of character objects (same shape as player, minus party/enemies/npcs fields).
</RPG_STATE FORMAT (REQUIRED)>
